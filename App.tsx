...
import { ActivityIndicator, Alert, Animated, Image, KeyboardAvoidingView, Linking, Modal, PanResponder, Platform, Pressable, SafeAreaView, ScrollView, StyleSheet, Text, TextInput, View } from 'react-native';
...
function RoutePin({ label, color }: { label: string; color: string }) {
  return <View style={styles.pinContainer}><View style={[styles.pinShape, { backgroundColor: color }]} /><Text style={styles.pinLabel}>{label}</Text></View>;
}
...
function DetailRows({ details, colors }: { details: PlaceDetails; colors: (typeof themes)[ThemeName] }) {
  ...
  return <View style={styles.detailRows}>
    ...
    <View style={styles.detailRow}><View style={styles.stars}>{Array.from({ length: 5 }, (_, index) => <Ionicons key={index} name={index < Math.round(details.rating ?? 0) ? 'star' : 'star-outline'} size={20} color={index < Math.round(details.rating ?? 0) ? '#fbbf24' : colors.muted} />)}</View><Text style={[styles.ratingText, { color: colors.text }]}>{details.rating ? `${details.rating} (${details.user_ratings_total ?? 0})` : 'No rating'}</Text></View>
...
function ItineraryRow({ place, index, total, isLast, colors, onOpen, onDrop }: { place: Place; index: number; total: number; isLast: boolean; colors: (typeof themes)[ThemeName]; onOpen: () => void; onDrop: (from: number, to: number) => void }) {
  ...
  const dragResponder = useMemo(() => PanResponder.create({
    onMoveShouldSetPanResponderCapture: () => { if (activeRef.current) capturedRef.current = true; return activeRef.current; },
    onMoveShouldSetPanResponder: () => activeRef.current,
    onPanResponderMove: (_, gesture) => { if (activeRef.current) { capturedRef.current = true; translateY.setValue(gesture.dy); } },
    onPanResponderRelease: (_, gesture) => {
      const target = Math.max(0, Math.min(total - 1, index + Math.round(gesture.dy / 106)));
      activeRef.current = false; capturedRef.current = false; setDragging(false);
      Animated.spring(translateY, { toValue: 0, damping: 18, stiffness: 220, useNativeDriver: true }).start();
      if (target !== index) { Haptics.selectionAsync(); onDrop(index, target); }
    },
    onPanResponderTerminate: () => { activeRef.current = false; capturedRef.current = false; setDragging(false); Animated.spring(translateY, { toValue: 0, useNativeDriver: true }).start(); },
  }), [index, total, translateY, onDrop]);
  const activateDrag = () => { activeRef.current = true; capturedRef.current = false; setDragging(true); Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium); Animated.spring(translateY, { toValue: -5, useNativeDriver: true }).start(); };
  const endStationaryHold = () => { if (activeRef.current && !capturedRef.current) { activeRef.current = false; setDragging(false); Animated.spring(translateY, { toValue: 0, useNativeDriver: true }).start(); } };
  return <Animated.View {...dragResponder.panHandlers} style={[styles.timelineRow, dragging && styles.draggingRow, { transform: [{ translateY }, { scale: dragging ? 1.025 : 1 }] }]}><View style={styles.timelineRail}><View style={[styles.timelineDot, { backgroundColor: stopColor }]}><Text style={styles.timelineNumber}>{index + 1}</Text></View>{!isLast && <View style={[styles.timelineLine, { backgroundColor: colors.border }]} />}</View><Pressable delayLongPress={350} onLongPress={activateDrag} onPressOut={endStationaryHold} onPress={() => { if (!activeRef.current) onOpen(); }} style={[styles.itineraryCard, dragging && styles.itineraryCardDragging, { backgroundColor: dragging ? colors.card : colors.surface, borderColor: dragging ? colors.primary : colors.border }]}><View style={styles.itineraryCopy}><View style={styles.itineraryMeta}><MaterialCommunityIcons name={place.stopType === 'pub' ? 'glass-mug-variant' : 'camera'} size={15} color={stopColor} /><Text style={[styles.itineraryType, { color: stopColor }]}>{place.stopType === 'pub' ? 'PUB' : 'ATTRACTION'}</Text></View><Text style={[styles.itineraryName, { color: colors.text }]} numberOfLines={2}>{place.name}</Text><Text style={[styles.itineraryAddress, { color: colors.muted }]} numberOfLines={1}>{dragging ? 'Move up or down, then release' : place.vicinity ?? 'Tap for place details'}</Text></View><MaterialCommunityIcons name={dragging ? 'drag-vertical' : 'gesture-tap-hold'} size={22} color={dragging ? colors.primary : colors.muted} /></Pressable></Animated.View>;
}
...
export default function App() {
  ...
  const dismissPlanner = () => Animated.timing(plannerTranslateY, { toValue: 700, duration: 220, useNativeDriver: true }).start(() => { setPlannerOpen(false); plannerTranslateY.setValue(0); });
  const plannerPanResponder = useMemo(() => PanResponder.create({
    ...
    onPanResponderMove: (_, gesture) => plannerTranslateY.setValue(Math.max(0, gesture.dy)),
    onPanResponderRelease: (_, gesture) => {
      if (gesture.dy > 90 || gesture.vy > 0.8) dismissPlanner();
      else Animated.spring(plannerTranslateY, { toValue: 0, damping: 18, stiffness: 220, useNativeDriver: true }).start();
    },
    onPanResponderTerminate: () => Animated.spring(plannerTranslateY, { toValue: 0, useNativeDriver: true }).start(),
  }), [plannerTranslateY]);

  const closeItinerary = () => Animated.timing(itineraryTranslateY, { toValue: 800, duration: 220, useNativeDriver: true }).start(() => { setItineraryOpen(false); setDetailsFromItinerary(false); setSelectedPlace(null); itineraryTranslateY.setValue(0); detailTranslateX.setValue(420); });
  const itineraryPanResponder = useMemo(() => PanResponder.create({
    onStartShouldSetPanResponder: () => true,
    onStartShouldSetPanResponderCapture: () => true,
    onMoveShouldSetPanResponderCapture: (_, gesture) => gesture.dy > 2 && Math.abs(gesture.dy) > Math.abs(gesture.dx),
    onMoveShouldSetPanResponder: (_, gesture) => gesture.dy > 2 && Math.abs(gesture.dy) > Math.abs(gesture.dx),
    onPanResponderMove: (_, gesture) => itineraryTranslateY.setValue(Math.max(0, gesture.dy)),
    onPanResponderRelease: (_, gesture) => {
      if (gesture.dy > 70 || gesture.vy > 0.65) closeItinerary();
      else Animated.spring(itineraryTranslateY, { toValue: 0, damping: 18, stiffness: 220, useNativeDriver: true }).start();
    },
    onPanResponderTerminate: () => Animated.spring(itineraryTranslateY, { toValue: 0, useNativeDriver: true }).start(),
  }), [itineraryTranslateY]);

  const openItineraryPlace = (place: Place) => { Haptics.selectionAsync(); setSelectedPlace(place); setDetailsFromItinerary(true); setItineraryOpen(true); itineraryTranslateY.setValue(0); detailTranslateX.setValue(420); requestAnimationFrame(() => Animated.spring(detailTranslateX, { toValue: 0, damping: 20, stiffness: 190, useNativeDriver: true }).start()); };
  const closeItineraryPlace = () => Animated.timing(detailTranslateX, { toValue: 420, duration: 200, useNativeDriver: true }).start(() => { setDetailsFromItinerary(false); setSelectedPlace(null); });
  ...
  return <View style={{ flex: 1, backgroundColor: colors.background }}>
    ...
    <MapView ref={mapRef} style={StyleSheet.absoluteFill} initialRegion={LONDON} provider={Platform.OS === 'android' ? PROVIDER_GOOGLE : undefined} customMapStyle={colors.map as any} userInterfaceStyle={themeName} showsUserLocation showsMyLocationButton={false}>
      {route && <><Polyline coordinates={route.coordinates} strokeColor={colors.primary} strokeWidth={6} /><Marker coordinate={route.origin} title="Start" anchor={{ x: 0.5, y: 1 }} zIndex={20}><RoutePin label="S" color="#2563eb" /></Marker>{route.stops.map((place, index) => <Marker key={place.place_id} coordinate={{ latitude: place.geometry.location.lat, longitude: place.geometry.location.lng }} anchor={{ x: 0.5, y: 1 }} zIndex={10 + index} onPress={() => openItineraryPlace(place)}><RoutePin label={String(index + 1)} color={place.stopType === 'pub' ? '#e11d48' : '#7c3aed'} /></Marker>)}<Marker coordinate={route.destination} title="Finish" anchor={{ x: 0.5, y: 1 }} zIndex={20}><RoutePin label="F" color="#16a34a" /></Marker></>}
    </MapView>

    <SafeAreaView pointerEvents="box-none" style={StyleSheet.absoluteFill}>
      <View style={styles.headerWrap}><View style={[styles.topCard, styles.headerPill, { backgroundColor: colors.card, borderColor: colors.border, shadowColor: colors.shadow }]}><View style={styles.headerRow}><View style={styles.logoBox}><Image source={require('./assets/tipsy-logo.png')} style={styles.brandLogo} resizeMode="contain" /></View><View style={{ flex: 1 }}><RainbowTitle /><View style={styles.taglineRow}><MaterialCommunityIcons name="map-marker-path" size={19} color={colors.muted} /><Text numberOfLines={1} style={[styles.tagline, { color: colors.muted }]}>{route ? `${route.distance} · ${route.duration} · ${travelLabel}` : 'Pubs, sights, one brilliant route'}</Text></View></View><Pressable accessibilityLabel="Open route planner" onPress={() => setPlannerOpen(true)} style={[styles.menuButton, styles.filterButton, { backgroundColor: colors.surface }]}><Ionicons name="options-outline" size={23} color={colors.primary} /></Pressable></View></View></View>
    ...
    <Modal visible={plannerOpen} transparent animationType="slide" onRequestClose={dismissPlanner}><KeyboardAvoidingView style={styles.sheetOverlay} behavior={Platform.OS === 'ios' ? 'padding' : undefined}><Pressable style={styles.sheetDismiss} onPress={dismissPlanner} /><Animated.View style={[styles.plannerSheet, { backgroundColor: colors.card, borderColor: colors.border, transform: [{ translateY: plannerTranslateY }] }]}><View style={styles.dragZone} {...plannerPanResponder.panHandlers}><View style={styles.sheetHandle} /><Text style={[styles.dragHint, { color: colors.muted }]}>Swipe down to close</Text></View><View style={styles.sheetHeader}><View><Text style={[styles.sheetEyebrow, { color: colors.primary }]}>BUILD A ROUTE</Text><Text style={[styles.sheetTitle, { color: colors.text }]}>Where are we going?</Text><Text style={[styles.sheetSubtitle, { color: colors.muted }]}>Choose your route and we’ll find the stops.</Text></View></View><ScrollView keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false} contentContainerStyle={styles.plannerContent}><View style={styles.routeInputs}><AutocompleteInput value={start} onChange={setStart} placeholder="Start location" onLocate={() => locateMe('start')} colors={colors} /><View style={[styles.routeConnector, { backgroundColor: colors.border }]} /><AutocompleteInput value={finish} onChange={setFinish} placeholder="Finish location" onLocate={() => locateMe('finish')} colors={colors} /></View><Text style={[styles.sectionLabel, { color: colors.muted }]}>TRAVEL MODE</Text><View style={[styles.modeRow, { backgroundColor: colors.surface, borderColor: colors.border }]}>{modes.map((item) => <Pressable key={item.value} onPress={() => setMode(item.value)} style={[styles.modeButton, mode === item.value && { backgroundColor: colors.primary }]}><MaterialCommunityIcons name={item.icon} size={20} color={mode === item.value ? '#fff' : colors.muted} /><Text style={[styles.modeLabel, { color: mode === item.value ? '#fff' : colors.text }]}>{item.label}</Text></Pressable>)}</View><Text style={[styles.sectionLabel, { color: colors.muted }]}>STOPS ALONG THE WAY</Text><View style={styles.countersRow}><StopCounter label="Pubs" max={7} value={pubs} icon="glass-cocktail" onChange={setPubs} colors={colors} /><StopCounter label="Sights" max={3} value={attractions} icon="camera" onChange={setAttractions} colors={colors} /></View><Pressable disabled={loading} onPress={submit} style={[styles.primaryButton, { backgroundColor: colors.primary }, loading && { opacity: 0.65 }]}>{loading ? <ActivityIndicator color="#fff" /> : <><MaterialCommunityIcons name="glass-mug-variant" size={21} color="#fff" /><Text style={styles.primaryButtonText}>{route ? 'Update my route' : 'Plan my Tipsy Tour'}</Text><Ionicons name="arrow-forward" size={21} color="#fff" /></>}</Pressable>{route && <Pressable onPress={clear} style={styles.sheetClear}><Ionicons name="trash-outline" size={18} color={colors.muted} /><Text style={[styles.sheetClearText, { color: colors.muted }]}>Clear current route</Text></Pressable>}</ScrollView></Animated.View></KeyboardAvoidingView></Modal>

    <Modal visible={itineraryOpen} transparent animationType="slide" onRequestClose={detailsFromItinerary ? closeItineraryPlace : closeItinerary}><View style={[styles.modalOverlay, styles.bottomModalOverlay]}><Pressable style={styles.modalDismiss} onPress={closeItinerary} /><SafeAreaView style={[styles.modalSafe, styles.bottomModalSafe]}><Animated.View style={[styles.modalPanel, styles.bottomModalPanel, styles.itineraryPanel, { backgroundColor: colors.card, borderColor: colors.border, transform: [{ translateY: itineraryTranslateY }] }]}><View style={styles.modalDragZone} {...itineraryPanResponder.panHandlers}><View style={styles.modalHandle} /></View>{detailsFromItinerary && selectedPlace ? <Animated.View style={[styles.drawerPage, { transform: [{ translateX: detailTranslateX }] }]}><View style={styles.modalHeader}><View style={styles.drawerTitleRow}><Pressable style={[styles.backButton, { backgroundColor: colors.surface }]} onPress={closeItineraryPlace}><Ionicons name="arrow-back" size={22} color={colors.text} /></Pressable><View><Text style={[styles.modalTitle, { color: colors.text }]}>Location details</Text><Text style={[styles.modalSubtitle, { color: colors.muted }]}>Stop {(route?.stops.findIndex((item) => item.place_id === selectedPlace.place_id) ?? 0) + 1} of {route?.stops.length}</Text></View></View></View><ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.locationContent}><PlaceCard place={selectedPlace} index={route?.stops.findIndex((item) => item.place_id === selectedPlace.place_id) ?? 0} colors={colors} /></ScrollView></Animated.View> : <View style={styles.drawerPage}><View style={styles.modalHeader}><View><Text style={[styles.modalTitle, { color: colors.text }]}>Your itinerary</Text><Text style={[styles.modalSubtitle, { color: colors.muted }]}>Hold a stop to reorder · tap for details</Text></View></View><View style={styles.summaryChips}><Text style={[styles.summaryChip, { backgroundColor: colors.surface, color: colors.text }]}>{route?.stops.length ?? 0} STOPS</Text><Text style={[styles.summaryChip, { backgroundColor: colors.surface, color: colors.text }]}>{route?.distance.toUpperCase()}</Text><Text style={[styles.summaryChip, { backgroundColor: colors.surface, color: colors.text }]}>{route?.duration.toUpperCase()}</Text><Text style={[styles.summaryChip, { backgroundColor: colors.primary, color: '#fff' }]}>{travelLabel.toUpperCase()}</Text></View><ScrollView style={styles.timelineScroll} showsVerticalScrollIndicator={false} contentContainerStyle={styles.timeline}>{route?.stops.map((place, index) => <ItineraryRow key={place.place_id} place={place} index={index} total={route.stops.length} isLast={index === route.stops.length - 1} colors={colors} onDrop={(from, to) => moveStop(from, to - from)} onOpen={() => openItineraryPlace(place)} />)}</ScrollView></View>}</Animated.View></SafeAreaView></View></Modal>
  </View>;
}
...
const styles = StyleSheet.create({
  ...
  sheetDismiss: { flex: 1 },
  plannerSheet: { maxHeight: '88%', borderTopLeftRadius: 30, borderTopRightRadius: 30, borderWidth: 1, borderBottomWidth: 0, overflow: 'hidden' },
  dragZone: { minHeight: 52, alignItems: 'center', justifyContent: 'center', paddingTop: 7 },
  sheetHandle: { width: 44, height: 5, borderRadius: 3, backgroundColor: '#cbd5e1' },
  dragHint: { fontSize: 10, fontWeight: '600', marginTop: 4 },
  sheetHeader: { flexDirection: 'row', alignItems: 'flex-start', justifyContent: 'space-between', paddingHorizontal: 20, paddingTop: 18, paddingBottom: 14 },
  sheetEyebrow: { fontSize: 11, fontWeight: '800', letterSpacing: 1.5, marginBottom: 5 },
  sheetTitle: { fontSize: 26, fontWeight: '800', letterSpacing: -0.6 },
  sheetSubtitle: { fontSize: 14, marginTop: 4 },
  plannerContent: { paddingHorizontal: 20, paddingBottom: Platform.OS === 'ios' ? 34 : 24, gap: 12 },
  routeInputs: { gap: 8, position: 'relative' },
  routeConnector: { position: 'absolute', left: 25, top: 48, width: 2, height: 18, zIndex: -1 },
  sectionLabel: { fontSize: 11, fontWeight: '800', letterSpacing: 1.2, marginTop: 5 },
  sheetClear: { minHeight: 42, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 7 },
  sheetClearText: { fontSize: 14, fontWeight: '600' },
  stars: { flexDirection: 'row', gap: 2 },
  ratingText: { fontSize: 15, fontWeight: '600' },
  bottomModalOverlay: { justifyContent: 'flex-end', padding: 0, backgroundColor: 'transparent' },
  bottomModalSafe: { flex: 0, width: '100%', height: '82%', maxHeight: '82%', justifyContent: 'flex-end' },
  bottomModalPanel: { height: '100%', maxHeight: '100%', borderBottomLeftRadius: 0, borderBottomRightRadius: 0, borderTopLeftRadius: 30, borderTopRightRadius: 30, borderBottomWidth: 0 },
  ...
  modalDismiss: { flex: 1 },
  modalHandle: { width: 42, height: 5, borderRadius: 3, backgroundColor: '#cbd5e1', alignSelf: 'center', marginTop: 10 },
  modalSubtitle: { fontSize: 13, marginTop: 3 },
  timeline: { paddingHorizontal: 18, paddingTop: 16, paddingBottom: 30 },
  timelineScroll: { flexGrow: 0, flexShrink: 1 },
  ...
  itineraryCard: { flex: 1, minHeight: 92, marginBottom: 14, borderWidth: 1, borderRadius: 16, padding: 12, flexDirection: 'row', alignItems: 'center', gap: 9 },
  itineraryCopy: { flex: 1 },
  itineraryMeta: { flexDirection: 'row', alignItems: 'center', gap: 5, marginBottom: 4 },
