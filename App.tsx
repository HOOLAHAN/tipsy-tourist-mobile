...
function RoutePin({ label, color }: { label: string; color: string }) {
  return <View style={styles.pinContainer}><View style={[styles.pinShape, { backgroundColor: color }]}><Text style={styles.pinLabel}>{label}</Text></View></View>;
}
...
function ItineraryRow({ place, index, isLast, colors, onOpen, onMove }: { place: Place; index: number; isLast: boolean; colors: (typeof themes)[ThemeName]; onOpen: () => void; onMove: (amount: number) => void }) {
  const stopColor = place.stopType === 'pub' ? '#e11d48' : '#7c3aed';
  return <View style={styles.timelineRow}><View style={styles.timelineRail}><View style={[styles.timelineDot, { backgroundColor: stopColor }]}><Text style={styles.timelineNumber}>{index + 1}</Text></View>{!isLast && <View style={[styles.timelineLine, { backgroundColor: colors.border }]} />}</View><Pressable onPress={onOpen} style={[styles.itineraryCard, { backgroundColor: colors.surface, borderColor: colors.border }]}><View style={styles.itineraryCopy}><View style={styles.itineraryMeta}><MaterialCommunityIcons name={place.stopType === 'pub' ? 'glass-mug-variant' : 'camera'} size={15} color={stopColor} /><Text style={[styles.itineraryType, { color: stopColor }]}>{place.stopType === 'pub' ? 'PUB' : 'ATTRACTION'}</Text></View><Text style={[styles.itineraryName, { color: colors.text }]} numberOfLines={2}>{place.name}</Text><Text style={[styles.itineraryAddress, { color: colors.muted }]} numberOfLines={1}>{place.vicinity ?? 'Tap for place details'}</Text></View><View style={styles.rowActions}><Pressable disabled={index === 0} onPress={() => onMove(-1)} style={[styles.rowAction, { backgroundColor: colors.card, opacity: index === 0 ? 0.35 : 1 }]}><Ionicons name="chevron-up" size={19} color={colors.text} /></Pressable><Pressable disabled={isLast} onPress={() => onMove(1)} style={[styles.rowAction, { backgroundColor: colors.card, opacity: isLast ? 0.35 : 1 }]}><Ionicons name="chevron-down" size={19} color={colors.text} /></Pressable></View><Ionicons name="chevron-forward" size={18} color={colors.muted} /></Pressable></View>;
}
...
export default function App() {
  ...
  const openItineraryPlace = (place: Place) => { setSelectedPlace(place); setDetailsFromItinerary(true); detailTranslateX.setValue(420); requestAnimationFrame(() => Animated.spring(detailTranslateX, { toValue: 0, damping: 20, stiffness: 190, useNativeDriver: true }).start()); };
...
const styles = StyleSheet.create({
  ...
  sheetOverlay: { flex: 1, justifyContent: 'flex-end', backgroundColor: 'rgba(3,8,20,0.48)' },
  sheetDismiss: { flex: 1 },
  ...
  bottomModalOverlay: { justifyContent: 'flex-end', padding: 0 },
  bottomModalSafe: { flex: 0, height: '94%', justifyContent: 'flex-end' },
  ...
  modalDragZone: { height: 32, alignItems: 'center', justifyContent: 'center' },
  drawerPage: { flex: 1 },
  ...
  pinShape: { width: 36, height: 36, borderRadius: 18, borderBottomRightRadius: 4, borderWidth: 2.5, borderColor: '#fff', alignItems: 'center', justifyContent: 'center', transform: [{ rotate: '45deg' }], shadowColor: '#000', shadowOpacity: 0.28, shadowRadius: 4, shadowOffset: { width: 0, height: 3 }, elevation: 5 },
  pinLabel: { color: '#fff', fontSize: 14, fontWeight: '900', transform: [{ rotate: '-45deg' }] },
